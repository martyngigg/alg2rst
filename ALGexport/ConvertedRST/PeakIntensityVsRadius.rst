...
.. figure:: PeakIntensityVsRadius_fig.png
...
`FindPeaksMD <FindPeaksMD>`__, for example.
...
-  Each peak is labeled with a string with "H K L". Use
   `IndexPeaks <IndexPeaks>`__ to automatically find HKL values for
   peaks.

This algorithm calls `IntegratePeaksMD <IntegratePeaksMD>`__ repeatedly,
